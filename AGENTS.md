...
Never commit credentials or `.p12` blobs. Feed the secrets referenced in `deploy/build_*` via environment variables, and prefer local keychains over repo storage. `version.h` is generated from `version.h.in`; change only the template tokens and let CMake emit the concrete header during configure.
