                                      ...
                                      ErrorCode &errorCode)
{
    ...
    // Add I1-I5 parameters (note: in config they are named "I1", "I2", etc. - the string literals, not the const names)
    jsonConfig[config_key::specialJunk1] = configMap.value("I1");
    jsonConfig[config_key::specialJunk2] = configMap.value("I2");
    jsonConfig[config_key::specialJunk3] = configMap.value("I3");
    jsonConfig[config_key::specialJunk4] = configMap.value("I4");
    jsonConfig[config_key::specialJunk5] = configMap.value("I5");

    // Note: J1-J3 (controlledJunk1-3) and Itime (specialHandshakeTimeout) are deprecated in AmneziaWG 2.0
    // They are intentionally not extracted to avoid conflicts with protocol version 2.0
