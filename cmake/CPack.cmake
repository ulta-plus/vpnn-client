set(CPACK_PACKAGE_VENDOR            VPNNaruzhu)
set(CPACK_PACKAGE_VERSION           ${VPNN_VERSION})
if(WIN32)
    set(CPACK_PACKAGE_FILE_NAME "VPNNaruzhu_${VPNN_VERSION}_windows_x64")
elseif(APPLE AND NOT IOS AND NOT MACOS_NE)
    set(CPACK_PACKAGE_FILE_NAME "VPNNaruzhu_${VPNN_VERSION}_macos_x64")
elseif(LINUX AND NOT ANDROID)
    set(CPACK_PACKAGE_FILE_NAME "VPNNaruzhu_${VPNN_VERSION}_linux_x64")
endif()
set(CPACK_PACKAGE_INSTALL_DIRECTORY VPNNaruzhu)
set(CPACK_PACKAGE_EXECUTABLES       VPNNaruzhu VPNNaruzhu)
...
list(PREPEND CPACK_COMPONENTS_ALL VPNNaruzhu)
...
set(CPACK_IFW_PACKAGE_NAME                          VPNNaruzhu)
set(CPACK_IFW_PACKAGE_TITLE                         VPNNaruzhu)
...
set(CPACK_PRODUCTBUILD_IDENTIFIER       org.vpnnaruzhu)
set(CPACK_PREFLIGHT_VPNNARUZHU_SCRIPT   ${CMAKE_SOURCE_DIR}/deploy/data/macos/post_uninstall.sh)
set(CPACK_POSTFLIGHT_VPNNARUZHU_SCRIPT  ${CMAKE_SOURCE_DIR}/deploy/data/macos/post_install.sh)
...
if(LINUX AND NOT ANDROID)
    install(FILES
        ${CMAKE_SOURCE_DIR}/deploy/data/linux/VPNNaruzhu.service
        ${CMAKE_SOURCE_DIR}/deploy/data/linux/VPNNaruzhu.png
        ${CMAKE_SOURCE_DIR}/deploy/data/linux/VPNNaruzhu.desktop
        ${CMAKE_SOURCE_DIR}/deploy/data/linux/post_install.sh
        ${CMAKE_SOURCE_DIR}/deploy/data/linux/post_uninstall.sh
        DESTINATION "."
        COMPONENT VPNNaruzhu
    )
endif()

if(WIN32)
    set(CPACK_IFW_PACKAGE_ICON "${CMAKE_SOURCE_DIR}/deploy/data/windows/installer_icon.ico")
    set(CPACK_IFW_PACKAGE_WINDOW_ICON "${CMAKE_SOURCE_DIR}/deploy/data/windows/installer_icon.png")
    install(FILES
...
if (APPLE AND NOT IOS AND NOT MACOS_NE)
    install(FILES ${CMAKE_SOURCE_DIR}/deploy/data/macos/VPNNaruzhu.plist
        DESTINATION "VPNNaruzhu.app/Contents/Resources"
        COMPONENT VPNNaruzhu
    )
endif()

include(CPackIFW)
cpack_ifw_configure_component(VPNNaruzhu
    VERSION ${VPNN_VERSION}
...
cpack_add_component(Uninstall
    DISPLAY_NAME "Uninstall VPNNaruzhu"
    REQUIRES_ADMIN_RIGHTS
    DISABLED
)
