...
$QT_BIN_DIR/qt-cmake -S $PROJECT_DIR -DSOTKA_VERSION=$SOTKA_VERSION
...
$CQTDEPLOYER_DIR/cqtdeployer.sh -bin $BUILD_DIR/client/Sotka -qmake $QT_BIN_DIR/qmake -qmlDir $PROJECT_DIR/client/ui/qml/ -targetDir $APP_DIR/client/
$CQTDEPLOYER_DIR/cqtdeployer.sh -bin $BUILD_DIR/service/server/Sotka-service -qmake $QT_BIN_DIR/qmake -targetDir $APP_DIR/service/
...
$CQTDEPLOYER_DIR/binarycreator.sh --offline-only -v -c $BUILD_DIR/installer/config/linux.xml -p $BUILD_DIR/installer/packages -f $PROJECT_DIR/deploy/Sotka_Linux_Installer.bin
